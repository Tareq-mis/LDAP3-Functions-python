...
def conn_LDAP():
    server = Server('put your ldap server')
...
def get_all_entries():
    ...
    try:
        if conn_LDAP():
            ...
            server = Server('put your ldap server', get_info=ALL)
            conn = Connection(server, 'put your ldap server user', 'put your ldap server password', auto_bind=True)
            conn.search('put your ldap server dc', '(objectclass=person)')
...
def get_entry_by_UID():
    ...
    try:
        if conn_LDAP():
            ...
            server = Server('put your ldap server', get_info=ALL)
            conn = Connection(server, 'put your ldap server user', 'put your ldap server password', auto_bind=True)
            conn.search('put your ldap server dc', '(&(objectclass=person)(cn=' + cn + '))',
